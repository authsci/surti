<template>
  <div class="max-h-screen overflow-y-scroll boddIndexY">
            ...
            >
              <nuxt-link :to="event.path">
                <div class="people-aub">
                  <img
                    :data-src="`/Content/AUB/events/colloquium/${event.thumbnail}`"
                    class="object-cover"
                    style="
...
export default {
  async asyncData({ $content, params }) {
    const events = await $content('AUB/events/colloquium')
      .sortBy('slug')
      .fetch()
