    ...
    <div id="main" class="absolute w-full font-mono">
      ...
      <div class="flex flex-col items-center mb-5 text-3xl text-left">
        <div class="w-8/12 mt-10">
          <div class="mb-6 text-xl font-bold sm:text-2xl">Workshops</div>
          ...
          <div
            class="flex flex-wrap items-start content-start justify-start mt-5"
          >
            <div
              v-for="event in events"
              :key="event.slug"
              class="w-full mb-10 mr-5 sm:w-4/12"
            >
              <nuxt-link :to="event.path">
                <div class="people-auc">
                  <img
                    :data-src="`/Content/AUC/events/workshops/${event.thumbnail}`"
                    class="object-cover"
                    style="
...
export default {
  async asyncData({ $content, params }) {
    const events = await $content('AUC/events/workshops').sortBy('slug').fetch()
