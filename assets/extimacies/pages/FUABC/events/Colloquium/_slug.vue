<template>
  <div class="max-h-screen overflow-y-auto boddIndexZ">
    <div id="main" class="absolute w-full font-mono">
      <div class="flex flex-row items-end font-mono">
        ...
        <div class="w-full sm:w-9/12">
          ...
          <div class="relative flex flex-col items-start mt-16 align-top">
            ...
            <img
              class="w-full"
              :data-src="`/Content/FUABC/events/colloquium/${aucColloquium.cover}`"
              alt
              v-lazy-load
            />
          </div>
      ...
      >
        <div v-if="$nuxt.$route.path === '/FUABC/events/colloquium/003'">
          <div class="mt-10"></div>
        </div>
...
export default {
  async asyncData({ $content, params }) {
    const aucColloquium = await $content(
      `FUABC/events/colloquium/${params.slug}`
    ).fetch()
    const [prev, next] = await $content('FUABC/events/colloquium')
      .only(['title', 'slug'])
      .sortBy('title', 'asc')
