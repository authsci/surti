<template>
  <div class="max-h-screen overflow-y-auto boddIndexA">
    <div id="main" class="absolute w-full font-mono">
      <div class="flex flex-row items-end font-mono">
        ...
        <div class="w-full sm:w-9/12">
          ...
          <div class="relative flex flex-col items-start mt-16 align-top">
            ...
            <img
              class="w-full"
              :data-src="`/Content/IUA/events/lectures/${aucColloquium.cover}`"
              alt
              v-lazy-load
            />
          </div>
      ...
      <div v-if="$nuxt.$route.path === '/IUA/events/lectures/001'">
        <iualec />
      </div>
    </div>
  </div>
...
export default {
  async asyncData({ $content, params }) {
    const aucColloquium = await $content(
      `IUA/events/lectures/${params.slug}`
    ).fetch()
    const [prev, next] = await $content('IUA/events/lectures')
      .only(['title', 'slug'])
      .sortBy('title', 'asc')
